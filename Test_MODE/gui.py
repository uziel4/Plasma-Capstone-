...
class PlasmaReactorGUI:
    def __init__(self, root):
        ...
        # Lector DAQC2 real o simulado
        self.daq = DAQC2Reader(
            address=DAQC2_ADDRESS,
            channel=DAQC2_CHANNEL,
            test_mode=TEST_MODE
        )

        # Controlador relay real o simulado
        self.relay = RelayController(
            bus_number=RELAY_I2C_BUS,
            address=RELAY_I2C_ADDRESS,
            test_mode=TEST_MODE
        )
        ...
        # Por ahora la turbomolecular no está disponible
        self.turbo_available = False

        # Variables del timer de high voltage
        ...
        # Al cerrar, apaga relays de forma segura
    ...
    def build_roughing_panel(self):
        ...
        self.label(panel, "ROUGHING LINE VACUUM", 19).grid(row=0, column=0, sticky="ew", pady=(10, 6))

        self.roughing_var = tk.StringVar(value="WAITING FOR SENSOR")

        self.readout(panel, self.roughing_var, size=19).grid(row=1, column=0, sticky="ew", padx=14, pady=(0, 8))
    ...
    def build_environment_panel(self):
        ...
        self.label(panel, "REAL SENSOR / HARDWARE DATA", 15).grid(row=0, column=0, sticky="ew", pady=(6, 2))
    ...
    def target_progress_percent(self, current_torr, target_mtorr):
        ...
        start_torr = max(self.initial_torr, 1e-7)
        current_torr = max(current_torr, 1e-7)
    ...
    # ========================================================
    # LECTURA Y ACTUALIZACIÓN EN TIEMPO REAL
    # ========================================================
    ...
    def update_loop(self):
        self.daq.update_simulation(
            roughing=self.roughing_active,
            mass_flow=self.mass_flow_active,
            hold=self.hold_mode,
            target_torr=self.parse_target_mtorr(self.target_vacuum_var.get())
        )
        ...
        if sensor_ok:
            ...
            hardware_mode = "SIMULATION MODE" if self.daq.simulated or self.relay.simulated else "REAL HARDWARE MODE"

            self.env_var.set(
                f"MODE           : {hardware_mode}\n"
                f"DAQC2 A0       : {self.current_voltage:0.4f} V\n"
